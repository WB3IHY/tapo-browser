...
- **Live-view firmware sensitivity (open issue):** go2rtc's `tapo://` source on recent C520WS firmware (e.g. 1.3.4) drops the stream every ~20s (`multipart: NextPart: EOF`) and never delivers H264 SPS/PPS, so MSE shows a `128×96` placeholder and nothing renders. Control/recordings/downloads are unaffected. Likely fix: add an optional RTSP source (needs RTSP enabled on the camera). `live.js` shows a hint box when no frame decodes within ~12s.
