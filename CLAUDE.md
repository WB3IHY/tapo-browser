...
- **When debugging the HA Supervisor add-on, never inspect `data/` by SSH-ing to the host path.** `/addons/local/tapo-browser/` on the HAOS host is only the add-on's *source* (bind-mounted for building) — the running container's own filesystem (where `PLAYBACK_DIR`/`DOWNLOADS_DIR`/etc. actually live, e.g. `/app/data/...`) is a separate namespace not reachable at that host path, and `docker` isn't available via SSH on HAOS to exec into it directly. A check like `ls /addons/local/tapo-browser/data/.../*.ts` silently returns nothing regardless of the real state, which reads exactly like "no segments produced" - a real, session-wasting false negative this project hit once. Always verify runtime state through the app's own HTTP API (e.g. `GET /api/playback/{id}/status`, or fetching `video.m3u8` itself) instead of the host filesystem.
- **A playback session's `no_data_timeout=60.0` is not evidence of a stall by itself, and neither is a slow-moving `/status` poll.** A real recording segment can take close to its own real duration to fully stream (the camera doesn't necessarily deliver much faster than real-time) - don't conclude a session is "stuck" from a snapshot of `running:true` after e.g. ~75s on a ~170s clip; let it run to `running:false` (or check `error`) before drawing conclusions.
