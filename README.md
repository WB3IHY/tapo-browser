...
**Current work:** prototyping recording **thumbnail previews** and **direct playback**
(streaming a recording straight from the camera without downloading it first) — the two
things the official Tapo Android app can do that this tool can't yet. See `CLAUDE.md` for
where that stands. Live view and the in-browser camera controls panel that used to be part
of this app have been removed to keep the project focused on that goal.
...
`ffmpeg` helper program into a local `bin/` folder), then opens your browser at
...
together with your TP-Link account password for everything this app does (recordings and
downloads).
...
password. `ffmpeg` converts downloaded recordings to `.mp4`. Camera list and download
history live in a local SQLite database. Inspired by the excellent
[HomeAssistant-Tapo-Control](https://github.com/JurajNyiri/HomeAssistant-Tapo-Control) —
the thumbnail/direct-playback work in progress here is aimed at eventually being
